...
class HierarchicalMemoryStore:
    ...
    async def _create_memory_summary(self, memories: list[dict[str, Any]]) -> dict[str, Any] | None:
...
Common Patterns:
- Session involved {len({mem['metadata'].get('session_id', '') for mem in memories})} sessions
...
"""
    ...
    async def _extract_essential_facts(self, summaries: list[dict[str, Any]]) -> dict[str, Any] | None:
        """Extract key facts from summaries."""

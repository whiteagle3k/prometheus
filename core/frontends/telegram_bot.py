...
from ..runtime.registry import get_agent, get_running_agents, add_shutdown_handler
...
Just send a message to chat! 🤖"""
    ...
    async def entities_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text(
            f"🤖 Доступные сущности / Available entities:\n\n" +
            "\n".join(entities_list) +
...
⏱️ Ограниченных пользователей: {active_timeouts}"""
            ...
            await update.message.reply_text(f"❌ Ошибка получения статуса / Error getting status: {str(e)}")
